...
def main() -> int:
    ...
    parser.add_argument(
        "command", nargs="?", choices=["single", "batch", "interactive", "batch-flow", "info"], help="运行模式"
    )
    ...
    if args.command == "single":
        ...
        if args.stocks_file:
            # 从文件读取股票列表
            try:
                with open(args.stocks_file, encoding="utf-8") as f:
                    for line in f:
                        line = line.strip()
                        if line and not line.startswith("#"):
                            parts = line.split(",")
                            if len(parts) == 2:
                                stocks.append({"company": parts[0].strip(), "ticker": parts[1].strip()})
