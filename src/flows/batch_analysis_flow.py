...
class BatchAnalysisFlow(Flow[BatchAnalysisState]):
    ...
    @start()
    def initialize_batch_analysis(self) -> dict[str, Any]:
        """初始化批量分析"""
    ...
    def validate_stock_list(self, config_data: dict[str, Any]) -> dict[str, Any]:
        """验证股票列表"""
        logger.info("=== 验证股票列表 ===")
    ...
    def execute_batch_analysis(self, validation_result: dict[str, Any]) -> dict[str, Any]:
        """执行批量分析"""
        logger.info("=== 执行批量分析 ===")
        ...
        try:
            ...
            # 执行批量分析
            result = self.batch_analyzer.analyze_multiple_stocks(self.state.stocks, strategy=self.state.strategy)
    ...
    def generate_batch_summary(self, analysis_result: dict[str, Any]) -> dict[str, Any]:
        """生成批量分析摘要"""
    ...
    def _generate_summary_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (self.state.success_count / len(self.state.stocks)) * 100 if self.state.stocks else 0,
            "average_score": (
                sum(result.get("overall_score", 0) for result in self.state.results.values()) / len(self.state.results)
            )
            ...
            "top_performers": [
                ...
                for ticker, result in sorted(
                    self.state.results.items(), key=lambda x: x[1].get("overall_score", 0), reverse=True
                )[:3]
            ],
            "bottom_performers": [
                {"ticker": ticker, "score": result.get("overall_score", 0)}
    ...
    def run_batch_analysis(
        self, stocks: list[dict[str, str]], strategy: str = "parallel", max_workers: int = 5
    ) -> dict[str, Any]:
        """运行批量分析"""
        self.state.stocks = stocks
