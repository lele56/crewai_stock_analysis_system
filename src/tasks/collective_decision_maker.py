...
from .task_dataclasses import DecisionType, VotingRecord, AgentCapability
...
class CollectiveDecisionMaker:
    ...
    def cast_vote(self, voter: str, vote: str, confidence: float,
                  reasoning: str = "") -> VotingRecord:
        record = VotingRecord(
            voter=voter,
            vote=vote,
    ...
    def decide(self, votes: List[VotingRecord]) -> Dict[str, Any]:
        ...
        decision = {
            ...
            "voting_details": [
                {"voter": v.voter, "vote": v.vote, "confidence": v.confidence}
                for v in votes
            ],
        }
...
_global_decision_maker: Optional[CollectiveDecisionMaker] = None
...
def create_investment_decision_vote(voter: str, score: float, confidence: float,
                                    reasoning: str = "") -> VotingRecord:
    if score >= 80:
        vote = "强烈买入"
    elif score >= 60:
