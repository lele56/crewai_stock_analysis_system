...
def generate_batch_summary(results: dict, errors: list, progress: dict) -> dict[str, Any]:
    ...
    summary["top_performers"] = [
        {"ticker": ticker, "score": result.get("overall_score", 0)} for ticker, result in sorted_results[:5]
    ]
...
def export_batch_results(results: dict, errors: list, export_format: str = "json", filepath: str = "") -> str:
    """导出批量分析结果"""
    if not filepath:
