...
class TestStockAnalysisSystem(unittest.TestCase):
    ...
    def test_analyze_stock_success(self, mock_crew):
        ...
        mock_crew_instance.execute_data_collection.return_value = {"success": True, "data": {"test": "data"}}
        mock_crew.return_value = mock_crew_instance

        # 模拟分析团队
        with patch.object(self.system.analysis_crew, "execute_analysis") as mock_analysis:
            mock_analysis.return_value = {"success": True, "data": {"analysis": "result"}}

            # 模拟决策团队
            with patch.object(self.system.decision_crew, "execute_decision_process") as mock_decision:
                mock_decision.return_value = {"success": True, "data": {"decision": "result"}}
    ...
    def test_analyze_stock_collection_failure(self, mock_crew):
        """测试数据收集失败的情况"""
        mock_crew_instance = Mock()
        mock_crew_instance.execute_data_collection.return_value = {"success": False, "error": "数据收集失败"}
        mock_crew.return_value = mock_crew_instance
    ...
    def test_analyze_multiple_stocks(self):
        """测试批量分析"""
        stocks = [{"company": "公司1", "ticker": "TICKER1"}, {"company": "公司2", "ticker": "TICKER2"}]
    ...
    def test_cache_functionality(self):
        """测试缓存功能"""
        # 添加数据到缓存
        self.system.cache["TEST"] = {"data": {"cached": "data"}, "timestamp": datetime.now().timestamp()}
    ...
    def test_generate_summary_report(self):
        ...
        results = [
            ...
            {"success": False, "company": "公司2", "ticker": "TICKER2", "error": "分析失败"},
        ]
...
class TestDecisionCrew(unittest.TestCase):
    ...
    def test_get_investment_rating(self):
        """测试投资评级获取"""
        decision_data = {"investment_recommendation": {"analysis_text": "基于综合分析，建议买入该股票"}}
