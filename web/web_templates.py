...
<head>
    ...
    <style>
        ...
        .tab { padding: 10px 20px; cursor: pointer; border: 1px solid #ddd; border-bottom: none; background-color: #f9f9f9; margin-right: 5px; }
        .tab.active { background-color: white; border-bottom: 2px solid white; margin-bottom: -2px; }
        ...
        .progress { width: 100%; height: 20px; background-color: #e0e0e0; border-radius: 10px; overflow: hidden; margin: 10px 0; }
        ...
        .stock-item { border: 1px solid #ddd; padding: 15px; border-radius: 4px; background-color: #f9f9f9; }
        .monitoring-status { margin-top: 20px; padding: 15px; border: 1px solid #ddd; border-radius: 4px; background-color: #e7f3ff; }
    </style>
</head>
<body>
    <div class="container">
    ...
    <script>
        ...
        document.getElementById('batchForm').addEventListener('submit', function(e) {
            ...
            fetch('/batch_analyze', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(data) })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    resultDiv.innerHTML = `<div class="success"><h3>批量分析完成！</h3>
                        <p><strong>总计:</strong> ${data.total_count}</p><p><strong>成功:</strong> ${data.success_count}</p>
                        <p><strong>失败:</strong> ${data.failed_count}</p><p><strong>成功率:</strong> ${data.success_rate.toFixed(1)}%</p></div>`;
                } else {
        ...
        function toggleMonitoring() {
            ...
            fetch(endpoint, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({interval: 300}) })
            .then(response => response.json()).then(data => { if (data.success) updateMonitorStatus(); });
        }
        function updateMonitorStatus() {
            fetch('/monitor/status').then(response => response.json()).then(data => {
                const statusDiv = document.getElementById('monitorStatus');
                if (data.status.monitoring) {
                    statusDiv.innerHTML = `<p><strong>状态:</strong> 运行中</p>
                ...
                } else { statusDiv.innerHTML = '<p><strong>状态:</strong> 未启动</p>'; }
            });
        }
        document.getElementById('monitorForm').addEventListener('submit', function(e) {
            ...
            const formData = new FormData(e.target);
            const data = Object.fromEntries(formData);
            fetch('/monitor/add_stock', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(data) })
            .then(response => response.json()).then(data => { alert(data.success ? '股票已添加到监控列表' : '添加失败: ' + data.error); });
        });
            ...
            fetch('/history').then(response => response.json()).then(data => {
                ...
                if (data.success) {
                    ...
                    data.history.forEach(item => {
                        html += `<div class="stock-item"><h4>${item.company} (${item.ticker})</h4>
                            <p><strong>时间:</strong> ${item.timestamp}</p><p><strong>状态:</strong> ${item.success ? '成功' : '失败'}</p></div>`;
                    });
                    html += '</div>';
                    historyDiv.innerHTML = html;
                } else { historyDiv.innerHTML = '<div class="error">加载历史失败</div>'; }
            });
        }
